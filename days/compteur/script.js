...
let START = new Date('2024-06-07T11:30:00');

// Charger la date de rencontre depuis la personnalisation
function loadMeetingDate() {
  ...
  if (personalization.meetingDate) {
    ...
    START = new Date(personalization.meetingDate + 'T00:00:00');
    
    // Mettre à jour l'affichage
    const title = document.getElementById('title');
    const senderName = personalization.senderName || '??';
    const recipientName = personalization.recipientName || '??';
    title.textContent = `${recipientName} et ${senderName} sont ensemble depuis`;
    
    const dateDisplay = document.getElementById('sinceDate');
    const options = { year: 'numeric', month: 'long', day: 'numeric' };
    const dateStr = START.toLocaleDateString('fr-FR', options);
